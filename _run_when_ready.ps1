...
$srcPath = 'F:\Amber_solutions_Kira\bm_site\_cat.jpg'
$outDir  = 'F:\Amber_solutions_Kira\bm_site'

if (-not (Test-Path $srcPath)) { Write-Host "NOT FOUND: $srcPath"; exit 1 }
...
function MakeSquare($src, $size, $path) {
    ...
    $dx = [int](($size - $w) / 2); $dy = [int](($size - $h) / 2)
...
Write-Host "Saved favicon.ico — DONE"
$orig.Dispose()
